...
%type <no> expr NUM program ID BEGINNEW END Slist stmt input output asgn ifstmt whilestmt BREAK CONTINUE dowhilestmt repeatstmt
%token NUM PLUS MINUS MUL DIV EQUAL ID BEGINNEW END READ WRITE IF WHILE DO END ENDIF ENDWHILE ELSE THEN BREAK CONTINUE LE GE LT GT NE EQEQ REPEAT UNTIL
...
stmt : input {$$=$1;} | output {$$=$1;} | asgn {$$=$1;} | ifstmt {$$=$1;} | whilestmt {$$=$1;} | BREAK {$$=$1;} | CONTINUE {$$=$1;} | dowhilestmt {$$=$1;} | repeatstmt {$$=$1;};
input : READ '(' ID ')' {$$=createTree(0,1,"r",1,$3,NULL,NULL);};

output : WRITE '(' expr ')' {$$=createTree(0,1,"w",2,$3,NULL,$3);};

asgn : ID EQUAL expr  {$$=createTree(0,1,"=",4,$1,NULL,$3);}
     ;
...
dowhilestmt : DO Slist WHILE '(' expr ')' ENDWHILE {$$=createTree(0,1,"dowhile",8,$2,NULL,$5);};

repeatstmt : REPEAT Slist UNTIL '(' expr ')'{$$=createTree(0,1,"repeat",9,$2,NULL,$5);}; 
